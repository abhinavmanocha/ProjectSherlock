...
//      }
      ...
      num sinAlpha = sin(aOrB);
      num cosAlpha = cos(aOrB);
          ...
          "Y") // vertical line. All stains above the horizontal ==> all Z values less than stain Z value.
      {
        ...
        num sinGamma2 = (sin(radians(gamma2))).abs();
        num cosGamma2 = (cos(radians(gamma2))).abs();
  ...
  Future<List<Output>> findXYPlaneIntersections() async {
    ...
    for (int i in params.leftStains) {
      for (int j in params.rightStains) {
        if (params.detailed) {
          outputs.add(Output(
              "Finding the intersection of lines $i and line $j in the X-Y plane",
              type: Output.title));
        }
    ...
    for (int i in params.verticalStains) {
      for (int j in params.leftStains) {
        if (params.detailed) {
          outputs.add(Output(
              "Finding the intersection of the vertical line $i and line $j in the X-Y plane\n",
              type: Output.title));
        }
...
//   var denom     = (params.sample.bloodStains[line2].deltaX*params.sample.bloodStains[line1].deltaY) - (params.sample.bloodStains[line2].deltaY*params.sample.bloodStains[line1].deltaX);
    ...
    num numerator = (params.sample.bloodStains[line1].deltaY *
    ...
    num denom = (params.sample.bloodStains[line2].deltaY *
            params.sample.bloodStains[line1].deltaX) -
    ...
    // fudge to make sure !=0 but too small is just as bad
    if (denom.abs() > 1.0e-4) {
      num t = numerator / denom;
      num intX = params.sample.bloodStains[line2].x2 +
          (t * params.sample.bloodStains[line2].deltaX);
      num intY = params.sample.bloodStains[line2].y2 +
...
//
    ...
    num t = (params.sample.bloodStains[line1].y -
...
//
  Future<List<Output>> convergencePoint() async {
    ...
    int nPoints = 0;
    num x, y;
    num test_stddev; // For each of lines to be removed (if any) calculate the centre and std.dev. Smallest std.dev wins.
    num test_x; // Coordinates (x,y) of the centroid caclulated without line___
    num test_y;
    num bad_stddev; // For each of lines to be removed (if any) calculate the centre and std.dev. Smallest std.dev wins.
    num bad_x; // Coordinates (x,y) of the centroid caclulated without line___
    num bad_y;
    num distance;
    num sumX = 0.0;
    num sumY = 0.0;
...
//
  ...
  Future<List<Output>> closest2D() async {
    ...
    for (int i = 0; i < params.sample.numStains; i++) {
      ...
      num dx = params.sample.bloodStains[i].x2 - params.sample.bloodStains[i].x;
      num dy = params.sample.bloodStains[i].y2 - params.sample.bloodStains[i].y;
      num val = ((dy * params.convergenceX) -
      ...
      if (denom == 0) {
        ...
        params.sample.bloodStains[i].dist2D = val / denom;
      }
...
//
    ...
    if (params.detailed) {
      outputs
          .add(Output("Point Number, X_Coord, Y_Coord, Z_Coord, Include?,\n"));
    }
...
//   Now that we have t, plug it into the parametric equation for find the Z value of the AO.
        ...
        num y_diff = params.convergenceY - params.sample.bloodStains[i].y;
...
// Temp removal to see if the small delta_y issue can be resolved using X instead....
        ...
        String str =
            "${((i+1).toString() + ',').padRight(3, " ")} ${f.format(params.convergenceX)},"
        ...
        num dx = params.convergenceX - params.sample.bloodStains[i].x;
        num dy = params.convergenceY - params.sample.bloodStains[i].y;
        num dz = params.sample.bloodStains[i].height -
...
//
    for (int i = 0; i < params.sample.numStains; i++) {
      if (params.sample.bloodStains[i].include) {
        num diff = params.sample.bloodStains[i].height - avg_z;
        z_stddev += (diff * diff);
      }
    }
...
//
  ...
  Future<List<Output>> displayResults() async {
    ...
    for (int i = 0; i < params.sample.numStains; i++) {
      ...
      outputs.add(
          Output("${i + 1} ${f.format(params.sample.bloodStains[i].alpha)} "
